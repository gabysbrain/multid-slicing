...
time.data = data.frame(rbind(
  {
    # cube
    cube.data = expand.grid(x1=c(-1,1), x2=c(-1,1), x3=c(-1,1))
    time.hull.gen("json/cube.json", cube.data, filter.pareto=FALSE, n=n.slices)
  }, {
    # octahedron
    oct.data = data.frame(rbind(diag(3), -1 * diag(3)))
    names(oct.data) = c("x1", "x2", "x3")
    oct.data = norm.data(oct.data)
    time.hull.gen("json/octahedron.json", oct.data, filter.pareto=FALSE, n=n.slices)
  }, {
    # tesseract
    tess.data = expand.grid(x1=c(-1,1), x2=c(-1,1), x3=c(-1,1), x4=c(-1,1))
    time.hull.gen("json/4d_cube.json", tess.data, filter.pareto=FALSE, n=n.slices)
  }, {
    # 16-cell (4D octahedron)
    cell16.data = data.frame(rbind(diag(4), -1*diag(4)))
    names(cell16.data) = str_c("x", 1:4)
    time.hull.gen("json/16_cell.json", cell16.data, filter.pareto=FALSE, n=n.slices)
  }, {
    # 5-cube
    cube5.data = expand.grid(x1=c(-1,1), x2=c(-1,1), x3=c(-1,1), x4=c(-1,1), x5=c(-1,1))
    time.hull.gen("json/5d_cube.json", cube5.data, filter.pareto=FALSE, n=n.slices)
  }, {
    # 5-ortho
    ortho5.data = rbind(diag(5), -1*diag(5))
    names(ortho5.data) = str_c("x", 1:5)
    time.hull.gen("json/5d_ortho.json", ortho5.data, filter.pareto=FALSE, n=n.slices)
  }, {
    # klein bottle
    n = 4000
    ...
    time.hull.gen("json/klein.json", kb.data, filter.pareto=FALSE, n=n.slices)
  }
))
