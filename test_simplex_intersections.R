...
test_that("3d-embedded triangle in plane (intersection)", {
  ...
  expect_equal(as.vector(res$d1.min), c(2,1,1))
  expect_equal(as.vector(res$d1.max), c(3,2,3))
  expect_equal(as.vector(res$d2.min), c(2,1,1))
  expect_equal(as.vector(res$d2.max), c(1,2,1))
})
